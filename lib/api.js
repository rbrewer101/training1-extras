...
function postNodeV1(req, res, next) {

    node = neo.createNodes(req.body,  function (err, result) {

        // Note: Neo4j browser cypher command
        // to delete these things: match n where n.Name='Bub' delete n
        ...
        if (err) {
            ...
            console.log('Node saved to database:', result);
            res.json(200, result);
            return next();
        }
    });
...
function getNodesV1(req, res, next) {


    var searchTerm = req.getQuery().searchTerm
    console.log("api.js : getNodesV1 : req.getQuery().searchTerm : ", req.getQuery().searchTerm)
...
//    }};
    ...
    console.log("getNodesV1 : query : ", query, " and params ", params);

    neo.cypherRunQuery(query, params, function(err, result) {

        if(err) {

            res.send(500, "ERROR: getNodesV1 : find");
            return next();
        }
        else {

            if (result.length === 0) console.log('nada');
...
function deleteNodeV1(req, res, next) {


    var targetId = Number(req.getQuery().targetNodeId);

    console.log("api.js : deleteNodeV1 : targetId : ", targetId);
    ...
    var params = {props: targetId}

    var nodes = []

    console.log("deleteNodeV1 : query : ", query);

    neo.cypherRunQuery(query, params, function(err, result) {

        if(err) {

            console.log("deleteNodeV1 : error : ", err);
            res.send(500, "ERROR: deleteNodeV1");
            return next();
        }
        else {

            console.log("deleteNodeV1 : Deleted node: ", targetId);
...
function putNodeV1(req, res, next) {

    var targetId = req.body._id;

    console.log("api.js : putNodeV1 : targetId : ", targetId);
    ...
    var nodes = []
    console.log("putNodeV1 : query : ", query)

    neo.cypherRunQuery(query, params, function(err, result) {

        if(err) {

            console.log("putNodeV1 : error : ", err);
            res.send(500, "ERROR: putNodeV1");
            return next();
        }
        else {

            console.log("putNodeV1 : Updated node : ", targetId);
...
exports.postNodeV1 = postNodeV1;
exports.getNodesV1 = getNodesV1;
exports.deleteNodeV1 = deleteNodeV1;
exports.putNodeV1 = putNodeV1;
